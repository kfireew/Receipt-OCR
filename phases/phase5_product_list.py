...
class Phase5ProductList:
    ...
    def _print(self, message: str):
        try:
            # On Windows, try to set console encoding
            import sys
            ...
            try:
                ...
                try:
                    ...
                    print("[Message contains undisplayable characters]")
    ...
    def load_product_list(self) -> bool:
        ...
        try:
            ...
            self._print(f"Phase 5: Found {len(self.product_df.columns)} columns in Excel file")
            for i, col in enumerate(self.product_df.columns):
                self._print(f"  Column {i}: {repr(str(col))}")

            # Create mapping with stripped column names (handles trailing/leading spaces)
            self.column_mapping = {str(col).strip(): col for col in self.product_df.columns}

            # Also create mapping with exact column names for reference
            self.exact_column_mapping = {str(col): col for col in self.product_df.columns}

            # Required columns - normal Hebrew names
            required_normal = ['קוד פריט', 'תאור פריט', 'שם ספק ראשי']
            # Also check for RTL/reversed Hebrew names (in case of display issues)
            required_rtl = ['טירפ דוק', 'טירפ רואת', 'ישאר קפס םש']

            missing = []
            found_columns = {}

            # Check for normal Hebrew columns first
            for col in required_normal:
                if col in self.column_mapping:
                    found_columns[col] = self.column_mapping[col]
                    self._print(f"Phase 5: Found column '{col}' as '{self.column_mapping[col]}'")
                else:
                    missing.append(col)

            # If normal Hebrew columns not found, check for RTL versions
            if missing and missing == required_normal:
                self._print("Phase 5: Normal Hebrew columns not found, checking for RTL versions...")
                missing = []
                for rtl_col, normal_col in zip(required_rtl, required_normal):
                    if rtl_col in self.column_mapping:
                        found_columns[normal_col] = self.column_mapping[rtl_col]
                        self._print(f"Phase 5: Found RTL column '{rtl_col}' as '{self.column_mapping[rtl_col]}' (maps to '{normal_col}')")
                    else:
                        missing.append(normal_col)

            if missing:
                self._print(f"Phase 5: Missing required columns: {missing}")
                self._print(f"Phase 5: Available columns (stripped): {list(self.column_mapping.keys())}")
                return False

            # Store column references using found columns
            self.product_code_col = found_columns['קוד פריט']
            self.description_col = found_columns['תאור פריט']
            self.supplier_name_col = found_columns['שם ספק ראשי']

            self._print(f"Phase 5: Using columns - Product Code: '{self.product_code_col}', "
                       f"Description: '{self.description_col}', Supplier: '{self.supplier_name_col}'")

            # Check for barcode column (optional) - try both normal and RTL
            barcode_found = False
            ...
            elif 'דוקרב' in self.column_mapping:  # RTL version
                self.barcode_col = self.column_mapping['דוקרב']
                barcode_found = True
                self._print(f"Phase 5: Found RTL barcode column '{self.barcode_col}' (maps to 'ברקוד')")
            else:
            ...
            self.loaded = True
            self._print(f"Phase 5: Successfully loaded {len(self.product_df)} products")
            return True

        except Exception as e:
            self._print(f"Phase 5: Failed to load product list: {e}")
            import traceback
            self._print(f"Phase 5: Traceback: {traceback.format_exc()}")
            return False
